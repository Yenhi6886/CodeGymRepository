...
public class BlogController {
    ...
    @GetMapping("/search")
    public String ajaxSearch(@RequestParam("search") String search, Model model) {
        model.addAttribute("blogs", blogService.findByTitleContaining(search, Pageable.unpaged()).getContent());
        return "blog/search-results :: table";
    }

    @GetMapping("/load-more")
    public String loadMore(@RequestParam("page") int page, Model model) {
        Page<Blog> blogsPage = blogService.findAll(Pageable.ofSize(3).withPage(page));
        model.addAttribute("blogs", blogsPage.getContent());
        return "blog/load-more :: tbody";
    }
}
